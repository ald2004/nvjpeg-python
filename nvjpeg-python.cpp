...
NvJpegPythonHandle* NvJpegPython_createHandle(){
    ...
    checkCudaErrors(nvjpegCreate(NVJPEG_BACKEND_DEFAULT, &dev_allocator,&(handle->nv_handle)));
//    nvjpegJpegStateCreate(handle->nv_handle, &(handle->nv_statue));
    checkCudaErrors(nvjpegJpegStateCreate(handle->nv_handle, &(handle->nv_statue)));
//    nvjpegEncoderStateCreate(handle->nv_handle, &(handle->nv_enc_state), NULL);
    checkCudaErrors(nvjpegEncoderStateCreate(handle->nv_handle, &(handle->nv_enc_state), NULL));
    checkCudaErrors(nvjpegEncoderParamsCreate(handle->nv_handle, &(handle->encode_params), NULL));
    checkCudaErrors(nvjpegEncoderParamsSetQuality(handle->encode_params, 70, NULL));
    checkCudaErrors(nvjpegEncoderParamsSetOptimizedHuffman(handle->encode_params, 0, NULL));
//    printf("111111111111111111111111111111");fflush(stdout);
    return handle;
}

void NvJpegPython_destoryHandle(NvJpegPythonHandle** handle){
    checkCudaErrors(nvjpegEncoderParamsDestroy((*handle)->encode_params));
    checkCudaErrors(nvjpegEncoderStateDestroy((*handle)->nv_enc_state));
    checkCudaErrors(nvjpegJpegStateDestroy((*handle)->nv_statue));
    checkCudaErrors(nvjpegDestroy((*handle)->nv_handle));
    free(*handle);
    *handle = NULL;
}
...
//    printf("111111111111111111111111111111");fflush(stdout);
    ...
    if(pReturn == NULL)
        return NULL;
//printf("22222222222222222222222222222222");fflush(stdout);
    Py_INCREF(&ModuleInfo);
    PyModule_AddObject(pReturn, "NvJpeg", (PyObject*)&NvJpeg_ClassInfo);
//    printf("333333333333333333333333333333333333");fflush(stdout);
    import_array();
    return pReturn;
}
